...
app.mount("/static", StaticFiles(directory="app/static"), name="static")  # ← agregar esta línea
...
# ─────────────────────────────────────────
# Helper: construir URL de redirección
# con mensaje o error correctamente encodeado
# ─────────────────────────────────────────
...
def ui_inicio(request: Request, mensaje: Optional[str] = None, error: Optional[str] = None):
    ...
    return templates.TemplateResponse("index.html", {
        "request": request,
        "estudiantes": est_svc.obtener_todos(),   # usa la capa de servicio
...
def ui_crear_estudiante(
    ...
    try:
        ...
        return _redirect("/ui", mensaje=f"Estudiante '{nuevo['nombre']}' registrado correctamente")
...
def ui_profesores(request: Request, mensaje: Optional[str] = None, error: Optional[str] = None):
    ...
    return templates.TemplateResponse("profesores.html", {
        "request": request,
        "profesores": prof_svc.obtener_todos(),   # usa la capa de servicio
        "mensaje": mensaje,
        "error": error,
    })
...
def ui_crear_profesor(
    ...
    try:
        ...
        return _redirect("/ui/profesores", mensaje=f"Profesor '{nuevo['nombre']}' registrado correctamente")
    except HTTPException as e:
